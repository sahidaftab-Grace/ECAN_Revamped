...
async function migrateHomeContent() {
  try {
    console.log('Creating editable homepage content...')
    ...
    await pool.query(
      `INSERT INTO site_content (section, data, updated_at)
       VALUES ('about-page', $1, now())
       ON CONFLICT (section) DO NOTHING`,
      [defaultAboutPage],
    )

    console.log('Editable homepage content is ready.')
  } catch (err) {
    console.error('Homepage content migration failed:', err)
    process.exitCode = 1
  } finally {
    await pool.end()
  }
